...
<head>
        <title>IMS - Pick List Sales Order</title>
		...
		<caption>Sales Order - List</caption>
			...
			<tbody id="main">
			  <c:if test="${requestScope.model.tableMap!=null}">
				...
				<c:forEach var="so" items="${requestScope.model.tableMap}" varStatus="soCounter">
				    ...
				    <tr class="ganjil">
						...
						<td class="style1" class="mid">
						    <form method="post" action="Picking.htm">
								...
								<input type="hidden" name="salesOrderNumber" value="<c:out value="${so.value.salesOrderNumber}"/>"/>
								<input type="hidden" name="salesOrderDate" value="<c:out value="${so.value.salesOrderDate}"/>"/>
								<label><input type="submit" name="btnSelect" id="btnSelect" value="Select" /></label>
						    </form>
						</td>
						<td align="center">&nbsp;
			      			<c:if test="${so.value.salesOrderNumber ne null}">
			      				${so.value.salesOrderNumber}
			      			</c:if>
			      		</td>
			      		<td align="center">&nbsp;
			      			<c:if test="${so.value.salesOrderDate ne null}">
			      				${so.value.salesOrderDate}
			      			</c:if>
			      		</td>
