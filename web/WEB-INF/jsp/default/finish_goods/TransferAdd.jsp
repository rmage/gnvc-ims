...
<html>
    ...
    <body onload="input.resetForm(this);">
        ...
        <script>
            ...
            $("#tsDatePicker").datepicker({ dateFormat: "dd/mm/yy", altFormat: "yy-mm-dd", altField: "#tsDate", changeYear: true, changeMonth: true, onSelect: function(dateText, inst) { gnvs.validator.backDate($(this), dateText, inst); } });
            ...
            $("#fTransfer").bind("submit", function() {
                ...
                var header = $('#tsCode').val() + ':s:' + $('#tsType').val() + ':s:' + $('#tsDate').val() + ':s:' +
                        $('#tsFrom').val() + ':s:' + $('#tsTo').val() + ':s:' + $('#tsRemarks').val() + ':s:';

                $('tbody#detail tr').each(function() {
                    data = data + header + $(this).find('td:eq(4)').data('id') + ':s:' + $(this).data('item') + ':s:' + $(this).find('.qty').val() + ':s::se:';
                });
