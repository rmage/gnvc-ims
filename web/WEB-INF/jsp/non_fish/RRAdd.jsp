...
<html>
    <head>
        ...
        <title>Create &therefore; Receiving &therefore; IMS</title>
        ...
        <script>
            ...
            $('#save').bind('click', function() {
                ...
                $('#main input[type="checkbox"]:checked').each(function() {
                    ...
                    data = data + $('#rrCode').val() + ':s:' + $('#rrDate').val() + ':s:' + $('#poCode').val() + ':s:'
                            + $('#from').val() + ':s:' + $('#remarks').val() + ':s:'
                            + $r.data('prs') + ':s:' + $r.find('td:eq(2)').html() + ':s:' + $r.find('td:eq(3)').html() + ':s:'
                            + $r.find('.qtyGood').val() + ':s:' + $r.find('.qtyBad').val() + ':s:' + $r.find('td:eq(5)').html() + ':s::se:';
                });

                if (data !== '' && confirm('Save Receiving #' + $('#rrCode').val() + ' ?')) {
                    console.log(data);
