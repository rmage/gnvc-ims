...
public class FishSupplierDaoImpl extends AbstractDAO implements
    ...
    public List<FishSupplier> findAllAndPaging(int limit, int offset) {
        ...
        List<FishSupplier> resultList = jdbcTemplate.query(query, this, limit, offset);
        return resultList;
    }
    ...
    public List<FishSupplier> searchAndPaging(String supplierName, int limit, int offset) {
        String query = "DECLARE @LIMIT int, @OFFSET int "
                ...
                + "	SELECT ROW_NUMBER() OVER (ORDER BY id DESC) AS RowNum, * "
                + "	FROM " + getTableName() + " "
                + "   WHERE is_active = 'Y' AND name LIKE ? "
                + ") AS RowConstrainedResult "
                + "WHERE RowNum >= @OFFSET AND RowNum < @OFFSET + @LIMIT "
                + "ORDER BY RowNum";
    ...
    public FishSupplier findBySupplierCode(String supplierCode) throws DaoException {
        FishSupplier result = new FishSupplier();
        String sqlQuery = "select * from dbo.fish_supplier where code like '" + supplierCode + "'";

        result = jdbcTemplate.queryForObject(sqlQuery, this);
