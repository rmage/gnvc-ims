...
public class FishBalanceHistoryDaoImpl extends AbstractDAO implements
    ...
    public int insertActual(FishBalanceHistory dto) {
        String query = "INSERT INTO fish_balance_history_actual"
                + " (doc_no, batch_no, fish_type, storage, qty_in, qty_out, balance,"
                + " created_date, created_by, is_active, is_delete) VALUES (?,?,?,?,?,?,?,?,?,?,?); SELECT @@IDENTITY;";

        int primaryKey = jdbcTemplate.queryForInt(query,
                new Object[]{dto.getDocNo(), dto.getBatchNo(), dto.getFishType(),
                    dto.getStorage(), dto.getQtyIn(), dto.getQtyOut(), dto.getBalance(),
                    dto.getCreatedDate(), dto.getCreatedBy(), dto.getIsActive(), dto.getIsDelete()}
        );
        return primaryKey;
