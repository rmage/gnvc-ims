 ...
 *
 *         This is a really simple exporter with many assumptions on report. The
 *         report page is divided in a grid of 10 pixel x 10 pixel. In each of
 *         this pixels there is a char. Elements must be inserted into this grid
 *         (so elements must have coords and dims that are multiple of 10px).
 *
 */
public class JRTxtExporter extends JRAbstractExporter {
                ...
                if (element instanceof JRPrintText) {
                    ...
                    if (pt.getY() == actual_cut) {
                        ...
                        if (pt.getHeight() <= CHAR_UNIT_V) // Single line object
                        {
                            ...
                            line = replaceLineRegion(line, start_at, len, str,
                                    pt.getTextAlignment());
                        ...
                        } else // is a multiline...
                        {
                            ...
                            // Vector field_lines = getFieldLines(line_height +
                            // " " + pt.getHeight() + " " + max_field_lines +
                            // str,len);
                            ...
                            for (nl = 0; nl < field_lines.size(); nl++) {
                                ...
                                line = replaceLineRegion(line, start_at, len,
                                        text, pt.getTextAlignment());
    ...
    protected String replaceLineRegion(String line, int start_at, int len,
            String str, byte alignment) {
        int side = 0;
        ...
        return line.substring(0, start_at) + str
                + line.substring(start_at + len);
    }
