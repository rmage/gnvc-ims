...
public class StockInventoryDaoImpl extends AbstractDAO implements ParameterizedRowMapper<StockInventory>, StockInventoryDao {
    ...
    /**
     * Returns the rows from the stock_inventory table that matches the
    ...
    public StockInventory findByProductCode(String productCode) {
        StockInventory result = new StockInventory();
        String queryString;

        queryString = "select * from stock_inventory where product_code = ? and is_active = 'Y'";
        result = (StockInventory) jdbcTemplate.query(queryString, this, productCode).get(0);
