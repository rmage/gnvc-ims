...
public class UserRoleDaoImpl extends AbstractDAO implements ParameterizedRowMapper<UserRole>, UserRoleDao {
    ...
    /**
     * Returns all rows from the user_role table that match the criteria 'id =
     * :id'.
     */
    @Transactional
    ...
    public List<UserRole> findAll() throws UserRoleDaoException {
        try {
            return jdbcTemplate.query("SELECT a.id, role_code, role_name, role_level, b.department_name FROM " + getTableName()
                    + " a LEFT JOIN department b ON b.department_code = a.department_code where role_code not in ('SYSADMIN') AND b.is_active = 'Y' ORDER BY a.role_name", this);
        } catch (Exception e) {
            throw new UserRoleDaoException("Query failed", e);
        }
    ...
    /**
     * Returns the rows from the user_role table that matches the specified
     * primary-key value.
     */
    ...
    public int ajaxMaxPage(BigDecimal show) {
        return jdbcTemplate.queryForInt("SELECT CEILING(COUNT(id)/?) maxpage FROM " + getTableName(), show);
    }

    public List<UserRole> ajaxSearch(String where, String order, int page, int show) {
        return jdbcTemplate.query("DECLARE @page INT, @show INT "
                + "SELECT @page=?, @show=? "
                + "SELECT * FROM ("
                + "SELECT a.id, role_code, role_name, role_level, b.department_name, ROW_NUMBER() OVER (" + (order.isEmpty() ? "ORDER BY a.id" : order) + ") row FROM " + getTableName() + " a LEFT JOIN department b ON b.department_code = a.department_code "
                + (where.isEmpty() ? "where role_code not in ('SYSADMIN') " : where + " AND role_code not in ('SYSADMIN')")
                + ") list WHERE row BETWEEN (((@page - 1) * @show) + 1) AND (@page * @show)", this, page, show);
    }

    public UserRole findId(int id) {
        List<UserRole> roles = jdbcTemplate.query("SELECT id, role_code, role_name, role_level, department_code FROM " + getTableName() + " WHERE id = ?", this, id);
