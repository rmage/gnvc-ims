...
public class ProductCategoryDaoImpl extends AbstractDAO implements ParameterizedRowMapper<ProductCategory>, ProductCategoryDao {
    ...
    /**
     * Returns the rows from the product_category table that matches the
    ...
    public List<ProductCategory> ajaxSearch(String where, String order, int page, int show) {
        return jdbcTemplate.query("DECLARE @page INT, @show INT "
                + "SELECT @page=?, @show=? "
                + "SELECT * FROM ("
                + "SELECT id, category_code, category_name, is_active, ROW_NUMBER() OVER (" + (order.isEmpty() ? "ORDER BY id" : order) + ") row FROM " + getTableName() + " " + (where.isEmpty()? "WHERE is_active='Y'" : where + " AND is_active='Y'")
                + ") list WHERE row BETWEEN (((@page - 1) * @show) + 1) AND (@page * @show)", new ProductCategoryListMap(), page, show);
    }
    
    public ProductCategory findId(int id){
        List<ProductCategory> productCategorys = jdbcTemplate.query("SELECT id, category_code, category_name, is_active, is_delete, created_by, created_date, updated_by, updated_date FROM " + getTableName() + " WHERE id=?", this, id);
        return productCategorys.isEmpty() ? null : productCategorys.get(0);
    }
}
