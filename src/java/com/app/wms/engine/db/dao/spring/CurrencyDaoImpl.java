...
public class CurrencyDaoImpl extends AbstractDAO implements ParameterizedRowMapper<Currency>, CurrencyDao {
    ...
    /**
     * Returns all rows from the currency table that match the criteria 'id =
     * :id'.
     */
    @Transactional
    ...
    /**
     * Returns the rows from the currency table that matches the specified
     * primary-key value.
     */
    ...
    public List<Currency> ajaxSearch(String where, String order, int page, int show) {
        return jdbcTemplate.query("DECLARE @page INT, @show INT "
                + "SELECT @page=?, @show=? "
                + "SELECT * FROM ("
                + "SELECT id, currency_code, currency_name, currency_symbol, is_active, is_delete, created_by, created_date, updated_by, updated_date, ROW_NUMBER() OVER (" + (order.isEmpty() ? "ORDER BY id" : order) + ") row FROM " + getTableName() + " " + (where.isEmpty() ? "WHERE is_active='Y'" : where + " AND is_active='Y'")
                + ") list WHERE row BETWEEN (((@page - 1) * @show) + 1) AND (@page * @show)", this, page, show);
    }
    
    public Currency findId(int id){
        List<Currency> currencys = jdbcTemplate.query("SELECT * FROM " + getTableName() + " WHERE id=?", this, id);
        return currencys.isEmpty() ? null : currencys.get(0);
    }
}
