...
public class UomDaoImpl extends AbstractDAO implements ParameterizedRowMapper<Uom>, UomDao {
    ...
    /**
     * Returns all rows from the uom table that match the criteria 'uom_code =
     * :uomCode'.
     */
    @Transactional
    ...
    /**
     * Returns all rows from the uom table that match the criteria 'uom_name =
     * :uomName'.
     */
    @Transactional
    ...
    /**
     * Returns all rows from the uom table that match the criteria 'remarks =
     * :remarks'.
     */
    @Transactional
    ...
    /**
     * Returns all rows from the uom table that match the criteria 'is_active =
     * :isActive'.
     */
    @Transactional
    ...
    /**
     * Returns all rows from the uom table that match the criteria 'is_delete =
     * :isDelete'.
     */
    @Transactional
    ...
    /**
     * Returns all rows from the uom table that match the criteria 'created_by =
     * :createdBy'.
     */
    @Transactional
    ...
    /**
     * Returns all rows from the uom table that match the criteria 'created_date
     * = :createdDate'.
     */
    @Transactional
    ...
    /**
     * Returns all rows from the uom table that match the criteria 'updated_by =
     * :updatedBy'.
     */
    @Transactional
    ...
    /**
     * Returns all rows from the uom table that match the criteria 'updated_date
     * = :updatedDate'.
     */
    @Transactional
    ...
    /**
     * Returns the rows from the uom table that matches the specified
     * primary-key value.
     */
    ...
    public List<Uom> ajaxSearch(String where, String order, int page, int show){
        return jdbcTemplate.query("DECLARE @page INT, @show INT "
                + "SELECT @page=?, @show=? "
                + "SELECT * FROM ("
                + "SELECT id, uom_code, uom_name, remarks, is_active, is_delete, created_by, created_date, updated_by, updated_date, ROW_NUMBER() OVER (" + (order.isEmpty() ? "ORDER BY id" : order) + ") row FROM " + getTableName() + " " + (where.isEmpty()? "WHERE is_active='Y'" : where + " AND is_active='Y'")
                + ") list WHERE row BETWEEN (((@page - 1) * @show) + 1) AND (@page * @show)", this, page, show);
    }
    
    //Modifien 14 April 2014
    public Uom findId(int id){
        List<Uom> uoms = jdbcTemplate.query("SELECT * FROM " +getTableName()+ " WHERE id=?", this, id);
