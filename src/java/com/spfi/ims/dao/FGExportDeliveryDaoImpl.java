...
public class FGExportDeliveryDaoImpl extends AbstractDAO implements FGExportDeliveryDao {
    ...
    public int ajaxMaxPage(BigDecimal show, String where) {
        try {
            return jdbcTemplate.queryForInt("EXEC FG_EXPORT_DELIVER_MAX_PAGE ?, ?", show, where);
        } catch(Exception e) {
            e.printStackTrace();
            return 1;
        }
    ...
    public List<Map<String, Object>> ajaxSearch(int page, int show, String where, String order) {
        try {
            return jdbcTemplate.queryForList("EXEC FG_EXPORT_DELIVER_LIST ?, ?, ?, ?", page, show, where, order);
        } catch(Exception e) {
            e.printStackTrace();
    ...
    public List<Map<String, Object>> getLabelingMonitoring(String lmCode) {
        try {
            return jdbcTemplate.queryForList("EXEC FG_LABELING_MONITORING_BY_LMRCODE ?", lmCode);
        } catch(Exception e) {
            e.printStackTrace();
            return new ArrayList<Map<String, Object>>();
        }
    ...
    public void ajaxNSave(String data, String separatorColumn, String separatorRow, String createdBy, int isBackDate) {
        jdbcTemplate.update("EXEC FG_EXPORT_DELIVERY_CREATE ?, ?, ?, ?, ?", data, separatorColumn, separatorRow, createdBy, isBackDate);
    }
